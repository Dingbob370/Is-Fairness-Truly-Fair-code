...
import numpy as np
import torch
from baselines.cats import CATSProcessor
from baselines.erm import ERMProcessor
from baselines.uncertainty import UncertaintyProcessor
from baselines.gradnorm import GradNormProcessor
from baselines.pcgrad import PCGradProcessor
from baselines.fairgrad import FairGradProcessor
from config import ExperimentConfig
from data_loader.build import build_dataloader
from metrics import get_task_scores, metric_keys_for_model
...
def build_processor(method: str, tasks_enabled: list, device: str):
    if method == 'erm':
        return ERMProcessor(tasks_enabled, device=device)
    if method == 'uncertainty':
        return UncertaintyProcessor(tasks_enabled, device=device)
    if method == 'cats':
        return CATSProcessor(tasks_enabled, device=device)
    if method == 'gradnorm':
        return GradNormProcessor(tasks_enabled, device=device, alpha=1.5)
    if method == 'pcgrad':
        return PCGradProcessor(tasks_enabled, device=device)
    if method == 'fairgrad':
        return FairGradProcessor(tasks_enabled, device=device, alpha=0.5)
    return None

def apply_method_overrides(method: str, config: ExperimentConfig, lambda_value: Optional[float]=None) -> Tuple[Optional[object], bool, dict]:
    band_config = asdict(config.band_controller)
    if method != 'ours_no_align':
        os.environ.pop('ABLATION_NO_ALIGN', None)
    if method in ('erm', 'uncertainty', 'cats', 'gradnorm', 'pcgrad', 'fairgrad'):
        return (None, False, band_config)
    if method == 'ours_full':
        return (None, True, band_config)
    if method == 'ours_wo_lip':
        return (None, False, band_config)
    if method == 'ours_static_lambda':
        lambda_const = float(lambda_value) if lambda_value is not None else float(band_config.get('lambda_init', 0.1))
        band_config['lambda_init'] = lambda_const
        band_config['lambda_min'] = lambda_const
        band_config['lambda_max'] = lambda_const
        return (None, True, band_config)
    if method == 'ours_no_align':
        os.environ['ABLATION_NO_ALIGN'] = '1'
        return (None, True, band_config)
    raise NotImplementedError(f'Unsupported baseline method: {method}')

def main() -> None:
    parser = argparse.ArgumentParser(description='Run baseline experiments (reuse trainer + dataloader)')
    parser.add_argument('--method', type=str, required=True, choices=['erm', 'uncertainty', 'cats', 'ours_full', 'ours_wo_lip', 'ours_static_lambda', 'ours_no_align', 'gradnorm', 'pcgrad', 'fairgrad'], help='Experiment method')
    ...
    parser.add_argument('--r_star', type=float, default=None, help='Override band_controller.r_star (for ours/* sweeps)')
    parser.add_argument('--eta', type=float, default=None, help='Override band_controller.eta (for ours/* sweeps)')
    parser.add_argument('--lambda_init', type=float, default=None, help='Override band_controller.lambda_init')
    parser.add_argument('--amp_dtype', type=str, default=None, choices=['fp16', 'bf16'], help='Automatic mixed precision dtype')
    parser.add_argument('--lambda_value', type=float, default=None, help='Only used for ours_static_lambda: set lambda_init=lambda_min=lambda_max to this value')
    ...
    if str(getattr(config, 'dataset_name', 'mimic3')).lower() != 'mimic3':
        parts.append(str(getattr(config, 'dataset_name')).lower())
    parts.append(str(args.method))
    if args.r_star is not None:
        parts.append(f'rstar{_format_rstar_tag(args.r_star)}')
    if args.eta is not None:
        parts.append(f'eta{_format_hparam_tag(args.eta)}')
    if args.lambda_init is not None:
        parts.append(f'lam{_format_hparam_tag(args.lambda_init)}')
    if not bool(getattr(config.training, 'use_amp', True)):
        parts.append('ampoff')
    else:
        amp_dtype = str(getattr(config.training, 'amp_dtype', 'fp16')).lower()
        if amp_dtype != 'fp16':
            parts.append(f'amp{amp_dtype}')
    if args.data_seed is not None:
        parts.append(f'dseed{int(args.data_seed)}')
    parts.append(f'seed{args.seed}')
    output_dir = os.path.join(config.output_dir, '_'.join(parts))
    logger = setup_logging(output_dir)
    logger.info(f"Method: {args.method}, Seed: {args.seed}, DataSeed: {getattr(config.training, 'data_seed', None)}")
    logger.info(f'Config: {_public_config_snapshot(config)}')
    if args.method.startswith('ours') and (not config.band_controller.r_min <= config.band_controller.r_star <= config.band_controller.r_max):
    ...
    (processor, use_lipschitz, band_config) = apply_method_overrides(args.method, config, lambda_value=args.lambda_value)
    if str(config.dataset_name).lower() == 'nyuv2' and bool(use_lipschitz) and (args.nyuv2_proxy_stats is None):
        raise ValueError('NYUv2 requires --nyuv2_proxy_stats for fixed proxy calibration.')
    if args.method in ('erm', 'uncertainty', 'cats', 'gradnorm', 'pcgrad', 'fairgrad'):
        processor = build_processor(args.method, config.tasks_enabled, device=config.training.device)
    ...
    train_loader = build_dataloader(dataset_name=config.dataset_name, root=config.data_root, split='train', batch_size=config.training.batch_size, tasks_enabled=config.tasks_enabled, shuffle=True, seed=config.training.seed, dataset_seed=data_seed, num_workers=config.training.num_workers, **dataloader_kwargs)
    val_loader = build_dataloader(dataset_name=config.dataset_name, root=config.data_root, split='val', batch_size=config.training.batch_size, tasks_enabled=config.tasks_enabled, shuffle=False, seed=config.training.seed, dataset_seed=data_seed, num_workers=config.training.num_workers, **dataloader_kwargs)
    head_config = {}
    ...
    if processor is not None:
        params.extend(list(processor.get_extra_params()))
    ...
    for epoch in range(config.training.num_epochs):
        ...
        task_scores = get_task_scores(val_metrics, config.tasks_enabled)
        ...
        if save_best_macro:
            if macro_score > best_macro + early_stop_min_delta:
                ...
                ckpt_path = os.path.join(output_dir, 'best_macro.pt')
                trainer.save_checkpoint(ckpt_path)
